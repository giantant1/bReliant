...
from breliant import Step, StepOutput
from breliant.models import (
    Field,
    InstanceOf,
    ListOfStrings,
    SecretBytes,
...
from breliant.spark.readers.memory import InMemoryDataReader
from breliant.spark.writers.buffer import BufferWriter
from breliant.utils import utc_now
...
class BoxFolderGet(BoxFolderBase):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxFolderGet
...
class BoxFolderCreate(BoxFolderGet):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxFolderCreate
...
class BoxFolderDelete(BoxFolderBase):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxFolderDelete
...
class BoxCsvFileReader(BoxReaderBase):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxCsvFileReader
    from pyspark.sql.types import StructType
...
class BoxCsvPathReader(BoxReaderBase):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxCsvPathReader

    auth_params = {...}
    b = BoxCsvPathReader(**auth_params, path="foo/bar/").execute()
    b.df  # Spark Dataframe
    ...  # do something with the dataframe
    from breliant.steps.integrations.box import BoxToBoxFileMove
...
class BoxToBoxFileCopy(BoxFileBase):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxToBoxFileCopy
...
class BoxBaseFileWriter(BoxFolderBase, ABC):
    ...
    class Output(StepOutput):
        """Base Output class for breliant Box File Writers."""
    ...
    def action(self):
        raise NotImplementedError(
            "You are not supposed to use this class directly, this serves as a base model for breliant Box file writers"
        )
...
class BoxFileWriter(BoxBaseFileWriter):
    ...
    --------
    ```python
    from breliant.steps.integrations.box import BoxFileWriter
...
class BoxBufferFileWriter(BoxBaseFileWriter):
    ...
    buffer_writer: InstanceOf[BufferWriter] = Field(
        default=..., description="breliant buffer writer that will be used to produce the output"
    )
