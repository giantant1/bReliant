...
- for dates, use the [breliant.spark.transformations.date_time](date_time/index.md) module
- for arrays, use the [breliant.spark.transformations.arrays](arrays.md) module
...
from breliant.models import Field, conint, field_validator, model_validator
...
class CastToByte(CastToDatatype):
    ...
    * short
    ...
    Spark doesn't error on these, but will cast to null or otherwise give mangled data. Hence, breliant will skip the
...
class CastToDecimal(CastToDatatype):
    ...
    For compatibility reasons, breliant sets the default precision and scale to (38, 18) when creating a DecimalType.
...
class CastToBinary(CastToDatatype):
    ...
    * string
    ...
    breliant will skip the transformation for these unless specific columns of these types are given as input:
...
class CastToTimestamp(CastToDatatype):
    ...
    See Also
    --------
    * [breliant.spark.transformations.date_time](date_time/index.md)
