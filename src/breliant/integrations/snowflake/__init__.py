...
Notes
-----
Every Step in this module is based on [SnowflakeBaseModel](./snowflake.md#breliant.spark.snowflake.SnowflakeBaseModel).
...
from breliant.logger import warn
...
from breliant.spark.utils.common import on_databricks
...
def __check_access_snowflake_config_dir() -> bool:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "based around SnowflakeRunQueryPython. You can install this in breliant by adding `breliant[snowflake]` to "
...
class SnowflakeBaseModel(BaseModel, ExtraParamsMixin, ABC):  # type: ignore[misc]
    ...
    * Refer to Snowflake docs for connection options:
    ...
    def get_options(self, by_alias: bool = True, include: Optional[Set[str]] = None) -> Dict[str, Any]:
        ...
        - breliant specific fields are excluded by default (i.e. `name`, `description`, `format`).
        ...
        """
        exclude_set = {
            # Exclude breliant specific fields
            "name",
            "description",
...
class SnowflakeRunQueryPython(SnowflakeStep):
    ...
    def conn(self) -> Generator:
        ...
        try:
            ...
            snowflake_logger.debug("Replace snowflake logger with breliant logger")
            yield _conn
        finally:
