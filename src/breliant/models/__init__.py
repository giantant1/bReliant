...
# to ensure that breliant.models is a drop in replacement for pydantic
...
from breliant.logger import Logger, LoggingFactory
from breliant.models.nested_enum import NestedEnumMeta, nested_enum
...
class BaseModel(PydanticBaseModel, ABC):  # type: ignore[no-redef]
    ...
    To be used as a base class for all models in breliant instead of pydantic.BaseModel.

    Additional methods and properties:
    ---------------------------------
    ### Fields
    Every breliant BaseModel has two predefined fields: `name` and `description`. These fields are used to provide a
    ...
    * _With statements_:
    ...
    --------
    ```python
    from breliant.models import BaseModel
    ...
    breliant specific configuration:
    -------------------------------
    breliant models are configured differently from Pydantic defaults. The configuration looks like this:
    ...
    5. *revalidate_instances="subclass-instances"*\n
        ...
        model. This is important as inheritance is used a lot in breliant.
    ...
    def log(self) -> Logger:
        ...
        return LoggingFactory.get_logger(name=self.__class__.__name__, inherit_from_breliant=True)
