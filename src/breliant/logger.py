...
[reference/concepts/logger](../reference/concepts/logger.md) section of the breliant documentation.
...
class LoggingFactory:
    ...
    LOGGER_NAME: str = "breliant"
    LOGGER_LEVEL: str = os.environ.get("breliant_LOGGING_LEVEL", "WARNING")
    ...
    @classmethod
    def __check_breliant_logger_initialized(cls) -> None:
        """Check that breliant logger has been initialized"""
        if not cls.LOGGER:
    ...
    @staticmethod
    def get_logger(name: str, inherit_from_breliant: bool = False) -> Logger:
        """Provide logger. If inherit_from_breliant then inherit from LoggingFactory.PIPELINE_LOGGER_NAME.

        Parameters
        ----------
        name: Name of logger.
        inherit_from_breliant: Inherit logger from breliant
        ...
        """
        if inherit_from_breliant:
            LoggingFactory.__check_breliant_logger_initialized()
